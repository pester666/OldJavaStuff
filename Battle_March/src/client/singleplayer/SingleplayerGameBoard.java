...
import lib.Unit;
import lib.UnitTypes;

@SuppressWarnings("serial")
public class SingleplayerGameBoard extends JPanel {
    private int x = 0;
    private int y = 0;
    private int xTrue = 0;
    private int yTrue = 0;
    ...
    public SingleplayerGameBoard() {
        ...
        dlg.setTitle("Status-Log");
        dlg.setSize(600, 200);
        dlg.setAlwaysOnTop(false);
        dlg.add(scrPane);
        dlg.setLocation(0, 800);
        dlg.setVisible(true);
    }
    ...
    private void giveMouseCoordinatesAndMakeAction(int x, int y) {
        ...
        try {
            ...
            txt.append("There is no HashMap loaded at this time!\n");
            txt.append("\n");
        }
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        ClassLoader cl = SingleplayerGameBoard.class.getClassLoader();
        URL imageURL = null;

        for (int i = 0; i < envMap.length; i++) {
            for (int j = 0; j < envMap[i].length; j++) {
                imgLocation = envMap[i][j].name + ".PNG";
                imageURL = cl.getResource(imgLocation);
                if (imageURL != null) {
                    g.drawImage(new ImageIcon(imageURL).getImage(), i * 32, j * 32, null);
                } else {
                    g.drawImage(new ImageIcon(imgLocation).getImage(), i * 32, j * 32, null);
                }
            }
        }
            ...
            for (int j = 0; j < hashMap[i].length; j++) {
                if (hashMap[i][j] != null) {
                    if (!hashMap[i][j].equals(currentSelection)) {
                        imgLocation = "images/units/" + getYear(age) + "/" + hashMap[i][j].name + ".PNG";
                        imageURL = cl.getResource(imgLocation);
                        if (imageURL != null) {
                            g.drawImage(new ImageIcon(imageURL).getImage(), i * 32, j * 32, null);
                        } else {
                        ...
                        imgLocation = "images/units/" + getYear(age) + "/selected/" + hashMap[i][j].name + ".PNG";
                        imageURL = cl.getResource(imgLocation);
                        if (imageURL != null) {
                            g.drawImage(new ImageIcon(imageURL).getImage(), i * 32, j * 32, null);
                        } else {
    ...
    private void makeUnitMove(int x, int y) {
        if (hashMap[oldX / 32][oldY / 32].alreadyMoved) {
            txt.append("You already moved that unit!\n");
            txt.append("\n");
            return;
        }
        if(isUnitInCloseCombat(oldX/32, oldY/32)){
            txt.append("Your unit may not move out of close combat!\n");
            txt.append("\n");
            return;
        }
        ...
        if (x < 32 && x > 0 && y < 24 && y > 0) {
            if (envMap[x - 1][y - 1].walkable) {
                if (distance <= currentSelection.movement) {
                    ...
                    txt.append("That is too far for ur unit to move!\n");
                    txt.append("\n");
                }
            } else {
                txt.append("This tile is not walkable!\n");
                txt.append("\n");
            }
        }
    }
    ...
    private void makeUnitAttack(Unit attacker, Unit defender) {
        if (attacker.alreadyAttacked) {
            txt.append("You already attacked with this unit!\n");
            txt.append("\n");
            return;
        }
        int ap = oldX / 32 - clickedTileNrX;
        ...
        if(!los.hasUnitLOS()){
            txt.append("The target is out of sight!\n");
            txt.append("\n");
        ...
        if (distance == 1) {
            ...
            for (int i = 0; i < attacker.gear.rateOfFire; i++) {
                if (attacker.unitType == Constants.UNIT_INFANTRY || attacker.unitType == Constants.UNIT_ANTITANK) {
                    ...
                    if (abberation == 0) {
                        if(attacker.attacks == 2){
                            for (int j = -1; j < 2; j++) {
                                if (hashMap[clickedTileNrX - j][clickedTileNrY] != null) {
                                    txt.append("Your artillery has hitted " + hashMap[clickedTileNrX - j][clickedTileNrY].name + " owned by " + hashMap[clickedTileNrX - j][clickedTileNrY].playerName + "\n");
                                    if (c.wound(attacker, hashMap[clickedTileNrX - j][clickedTileNrY], false)) {
                                        if (c.coverSaveFails(attacker, hashMap[clickedTileNrX - j][clickedTileNrY], getDefendersCover(), false)) {
                                            targetGetsDamaged();
                                            if (age == Constants.WW2 || age == Constants.SCIFI) {
                                                hashMap[clickedTileNrX - j][clickedTileNrY] = explosion;
                                            } else if (age == Constants.FANTASY) {
                                                hashMap[clickedTileNrX - j][clickedTileNrY] = catapultRock;
                                            }
                                        }
                                    }
                                } else {
                                    ...
                                    txt.append("Your artillery has hitted an empty tile!\n");
                                    txt.append("\n");
                                }
                            }
                            for (int j2 = -1; j2 < 2; j2++) {
                                if (hashMap[clickedTileNrX][clickedTileNrY - j2] != null) {
                                    txt.append("Your artillery has hitted " + hashMap[clickedTileNrX][clickedTileNrY - j2].name + " owned by " + hashMap[clickedTileNrX][clickedTileNrY - j2].playerName + "\n");
                                    if (c.wound(attacker, hashMap[clickedTileNrX][clickedTileNrY - j2], false)) {
                                        if (c.coverSaveFails(attacker, hashMap[clickedTileNrX][clickedTileNrY - j2], getDefendersCover(), false)) {
                                            targetGetsDamaged();
                                            if (age == Constants.WW2 || age == Constants.SCIFI) {
                                                hashMap[clickedTileNrX][clickedTileNrY - j2] = explosion;
                                            } else if (age == Constants.FANTASY) {
                                                hashMap[clickedTileNrX][clickedTileNrY - j2] = catapultRock;
                                            }
                                        }
                                    }
                                } else {
                        ...
                        aquireNewArtilleryTarget(abberation+attacker.attacks-1);
                        if(attacker.attacks == 2){
                            for (int j = -1; j < 2; j++) {
                                if (hashMap[clickedTileNrX - j][clickedTileNrY] != null) {
                                    txt.append("Your artillery has hitted " + hashMap[clickedTileNrX - j][clickedTileNrY].name + " owned by " + hashMap[clickedTileNrX - j][clickedTileNrY].playerName + "\n");
                            ...
                            if (hashMap[clickedTileNrX][clickedTileNrY] != null) {
                                txt.append("Your artillery has hitted " + hashMap[clickedTileNrX][clickedTileNrY].name + " owned by " + hashMap[clickedTileNrX][clickedTileNrY].playerName + "\n");
                    ...
                    txt.append("Not yet implemented!\n");
                    txt.append("\n");
                    ...
                    for (int j = 0; j < attacker.attacks; j++) {
                        if (hashMap[clickedTileNrX][clickedTileNrY] != null && !hashMap[clickedTileNrX][clickedTileNrY].playerName.equals("Environment")) {
                            txt.append("Your machinegun has hitted " + hashMap[clickedTileNrX][clickedTileNrY].name + " owned by " + hashMap[clickedTileNrX][clickedTileNrY].playerName + "\n");
                            if (c.wound(attacker, hashMap[clickedTileNrX][clickedTileNrY], false)) {
                                if (c.coverSaveFails(attacker, hashMap[clickedTileNrX][clickedTileNrY], getDefendersCover(), false)) {
            ...
            txt.append("The target is out of range!\n");
            txt.append("\n");
        }
        hashMap[oldX / 32][oldY / 32].alreadyAttacked = true;
    }
    ...
    private void targetGetsDamaged() {
        ...
        if(hashMap[clickedTileNrX][clickedTileNrY].hp <= 0){
            ...
            txt.append("The target unit has lost one Hitpoint!\n");
            txt.append("\n");
        } 
    }
            ...
            if (isDeleting) {
                for (int i = 0; i < playerRessources.size(); i++) {
                    if (playerRessources.get(i).nameOfThePlayer.equals(selectedPlayer)) {
                        ...
                        txt.append(listSelection.name + " sold.\n");
                        txt.append("You have " + playerRessources.get(i).pointLimitForPlayer + " Points left to spend!\n");
                        txt.append("\n");
                    }
                }
            } else {
    ...
    private void createUnit(int x, int y) {
        if (listSelection != null) {
            if (envMap[x][y].walkable) {
                    ...
                    for (int i = 0; i < playerRessources.size(); i++) {
                        if (playerRessources.get(i).nameOfThePlayer.equals(selectedPlayer)) {
                            if (playerRessources.get(i).pointLimitForPlayer >= listSelection.costs) {
                                ...
                                txt.append(listSelection.name + " bought.\n");
                                txt.append(playerRessources.get(i).nameOfThePlayer + " has " + playerRessources.get(i).pointLimitForPlayer + " Points left to spend!\n");
                                txt.append("\n");

                            } else {
                                txt.append(playerRessources.get(i).nameOfThePlayer + " has insufficient founds!\n");
                                txt.append("\n");
                            }
                        }
                    }
                ...
                txt.append("You may not place a unit there!\n");
                txt.append("\n");
            }
        } else {
            txt.append("You have not selected a unit to deploy!\n");
            txt.append("\n");
        }
    }
