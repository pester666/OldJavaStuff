...
public class ChatServerLobby extends Thread{
...
//    public String[] playerRaces = new String[5];
    ...
    private void changeRaceForPlayer(String name, String race) {
        for (int i = 0; i < p.length; i++) {
            if(p[i].getName().equals(name)){
                p[i].setRace(race);
                break;
    ...
    private String infoMsgs(String nachricht, String[] frame) {
        ...
        } else if (frame[2].equals(RMI.RACE)) {
            changeRaceForPlayer(frame[1], frame[3]);
        }
